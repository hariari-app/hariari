...
describe('node-pty Runtime asset plan', () => {
  it(
    'requires executable Darwin spawn-helper beside pty.node for darwin-arm64',
    verifiesDarwinPlan,
  );
  it(
    'fails preflight clearly when the Darwin companion helper is absent',
    rejectsMissingDarwinHelper,
  );
  it(
    'makes the Darwin helper executable even when the installed prebuild mode is not',
    verifiesDarwinModes,
  );
  it(
    'keeps the Windows prebuild plan limited to loadable binaries and helpers',
    verifiesWindowsPlan,
  );
  it(
    'validates the Windows prebuild root once before selecting nested conpty executables',
    verifiesNestedWindowsAssets,
  );
});

function verifiesDarwinPlan(): void {
  ...
  expect(selectNodePtyNativeAssets(plan, ['pty.node', 'spawn-helper', 'ignored.pdb'])).toEqual([
    'pty.node',
    'spawn-helper',
  ]);
}

function rejectsMissingDarwinHelper(): void {
  const plan = nodePtyNativeAssetPlan({
    platform: 'darwin',
    arch: 'arm64',
    hasPrebuild: true,
...
function verifiesDarwinModes(): void {
  const plan = nodePtyNativeAssetPlan({
    platform: 'darwin',
    arch: 'arm64',
    hasPrebuild: true,
...
function verifiesWindowsPlan(): void {
  const plan = nodePtyNativeAssetPlan({
    platform: 'win32',
    arch: 'x64',
    hasPrebuild: true,
...
function verifiesNestedWindowsAssets(): void {
  const plan = nodePtyNativeAssetPlan({
    platform: 'win32',
    arch: 'x64',
    hasPrebuild: true,
