...
if (!x64Path || !arm64Path || !outputPath) {
  throw new Error('Expected x64, arm64, and output update manifest paths');
}

const x64 = splitFilesSection(fs.readFileSync(x64Path, 'utf8'));
const arm64 = splitFilesSection(fs.readFileSync(arm64Path, 'utf8'));
if (x64.prefix.join('\n') !== arm64.prefix.join('\n')) {
  throw new Error('Architecture update manifests disagree before their files sections');
}
const mergedFiles = [...x64.files, ...arm64.files];
if (mergedFiles.length < 2) throw new Error('Update manifests contain too few files');
fs.writeFileSync(
  outputPath,
...
function splitFilesSection(source) {
  ...
  if (start < 0) throw new Error('Update manifest has no files section');
  let end = start + 1;
